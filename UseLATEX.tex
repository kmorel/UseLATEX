...
ADD_LATEX_DOCUMENT(MyDoc.tex BIBFILES MyDoc.bib
  ...
  \textlatexpackage{glossaries} requires running a separate program,
  \textprog{makeglossaries}, for building auxiliary files.  Simply add the
  \textcmake{USE\_GLOSSARIES} option anywhere in the \ald arguments, and
  \textprog{makeglossaries} will automatically be added to the build.
...
ADD_LATEX_DOCUMENT(MyDoc.tex BIBFILES MyDoc.bib
  ...
  Note that the \textprog{makeglossaries} program is not included in many
  \latex distributions.  You may need to download it from the
  \textlatexpackage{glossaries} package source.
...
convert.exe: unable to open image `C:': Permission denied.
  ...
  \begin{description}
  \item[\O{}ystein S. Haaland] Support for makeglossaries command.
